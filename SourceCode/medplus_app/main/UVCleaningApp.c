 ...
 */
  void UvC_10msTask(void)
  {
	  switch(uvc_mode)
	  {
	  	  case UVC_MODE_IDLE:
	  		  ...
	  		  if(!MotorCtrl_isCompletelyClosed() || getTempState())
	  		  {
	  			LampCtrl_TurnOFF();
	  		  }
	  		  break;
  ...
  {
	  if(((!activeCleaning) && (MotorCtrl_isCompletelyClosed() == TRUE)) || getTempState())
	  {
		  //enableBMISensor();
		if(onoffTestCounter > 60  && onoffTestCounter < 120)
		{
			LampCtrl_TurnOFF();
		}
