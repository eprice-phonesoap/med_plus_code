...
@file			  wifi_tls.c
@author			  Jey
@brief			  WiFi TLS module related functions
			      WARNING: Do not use this module with multiple wifi_tls_context creation.
			     	 	   Content is modified only to work with one context (iap_https_init)
...
typedef struct wifi_tls_context_ {
    ...
    char server_host_name[100]; //Actual host name is less than this
    char server_root_ca_public_key_pem[4096]; //Actual certificate is less than this
    char peer_public_key_pem[4096]; //Actual certificate is less than this.
    int server_port;
...
static wifi_tls_context_t ctx = {0};
...
wifi_tls_context_t *wifi_tls_create_context(wifi_tls_init_struct_t *params)
{
    ...
    //context structure is statically created instead of dynamic allocation
#if 0
...
int wifi_tls_send_request(wifi_tls_context_t *ctx, wifi_tls_request_t *request)
{
    ...
    ESP_LOGI(TAG, "wifi_tls_send_request: '%s'", request->request_buffer);
    ...
    while (1) {
        ...
        wifi_tls_disconnect(ctx);
        return -1;
        //return 0; //Bypassing the error here
    }
