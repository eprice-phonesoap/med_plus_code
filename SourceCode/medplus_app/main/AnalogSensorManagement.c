 ...
 */
 void AsM_10msTask(void)
 {
	 ...
	 switch(AsM_ControlState)
	 {
		 case ASM_READ_CLEAN_TIME_KNOB:
			 ...
			 {
				 asm_tempValue = asm_tempValue + asm_temp_avg[i];
			 }

			 asm_finalValue = asm_tempValue/ASM_TEMP_AVG_NUM;
			 //asm_finalValue = asm_tempValue;
			 break;
 ...
 {
	 ESP_LOGI(TAG,"TEMPERATURE: %d HYST: %d TEMP_TEMP: %d",  (int)asm_finalValue, (int)asm_temp_hyst, (int)asm_finalValue);
